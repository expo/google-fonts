...
async function randomDrawListItem(x, y) {
  let word = randomWord();
...
export default function App() {
  ...
  useInterval(() => {
    (async () => {
      ...
      let item = await randomDrawListItem(x, y);
      setDrawList([...drawList.slice(Math.max(0, drawList.length - ItemLimit + 1), drawList.length), item]);
    })();
  }, 50);
  ...
  return (
    <TouchableWithoutFeedback
      onPress={async (e) => {
        ...
        console.log({ x, y });
        let newItem = await randomDrawListItem(x, y);
        console.log(newItem);
